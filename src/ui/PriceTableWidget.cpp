...
void PriceTableWidget::refresh() {
    ...
    for (int i = 0; i < tracked.size(); ++i) {
        ...
        PriceSnapshot snap = m_db->latestPriceSnapshot(item.id, item.quality);
        AnalysisResult analysis = m_analyzer->lastResult(item.id, item.quality);

        m_table->setItem(i, 0, new QTableWidgetItem(item.nameRu));
        m_table->setItem(i, 1, new QTableWidgetItem(Item::qualityName(item.quality)));
        m_table->setItem(i, 2, new QTableWidgetItem(QString::number(snap.minPrice)));
        m_table->setItem(i, 3, new QTableWidgetItem(QString::number(snap.avgPrice)));
        m_table->setItem(i, 4, new QTableWidgetItem(QString::number(snap.medianPrice)));
        m_table->setItem(i, 5, new QTableWidgetItem(QString::number(snap.maxPrice)));
        m_table->setItem(i, 6, new QTableWidgetItem(QString::number(snap.stdDev, 'f', 0)));
        m_table->setItem(i, 7, new QTableWidgetItem(QString::number(snap.lotCount)));
        m_table->setItem(i, 8, new QTableWidgetItem(QString::number(analysis.zScore, 'f', 2)));
        ...
        m_table->setItem(i, 9, new QTableWidgetItem(trendStr));
        m_table->setItem(i, 10, new QTableWidgetItem(QString::number(analysis.rating, 'f', 2)));
        ...
        m_table->setItem(i, 11, signalItem);
    }
